(function ($) {
    ...
    if ($('.datepicker').length) {
        $(".datepicker").datepicker({
            prevText: "Назад",   // Text for Previous button
            nextText: "Напред"    // Text for Next button
        });
    }
    ...
    if ($('.deleteCar').length) {
        ...
        const carsDeleteButton = document.querySelectorAll('.deleteCar');
        const backgroundModal = document.querySelector('.background-modal');
        const modal = document.querySelector('.custom-modal');

        let carDeleteInput = document.querySelector('.carDeleteInput');

        carsDeleteButton.forEach(button => {

            button.addEventListener('click', function (e) {

                const parent = button.parentElement;
                const carId = parent.querySelector('.carId').value;

                backgroundModal.style.display = 'flex';
                carDeleteInput.value = carId;
